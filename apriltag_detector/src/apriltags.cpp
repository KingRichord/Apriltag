...
static double getReprojectionError(const vector<Point3f>& objectPoints, const vector<Point2f>& imagePoints,
                                   const cv::Matx33d& cameraMatrix, const cv::Vec4f& distCoeffs, const Vec3d& rvec,
                                   const Vec3d& tvec) {
...
void ImageCallback(const sensor_msgs::ImageConstPtr& msg) {
  ...
  threshold(gray, gray, 150, 255, THRESH_OTSU);
...
int main(int argc, char** argv) {
  ...
  node.param("camera_topic", camera_topic_name_, DEFAULT_CAMERA_TOPIC_NAME);
  node.param("camera_info", camera_info_topic_name_, DEFAULT_CAMERA_INFO_NAME);


  node.param("default_tag_size", default_tag_size_, DEFAULT_TAG_SIZE);
  node.param("display_type", display_type_, DEFAULT_DISPLAY_TYPE);
  node.param("marker_thickness", marker_thickness_, 0.01);
  node.param("process_hz", process_hz_, 10.0);
  node.param("viewer", viewer_, false);
  node.param("debug", debug_, false);

  node.param("display_marker_overlay", display_marker_overlay_, true);
  node.param("display_marker_outline", display_marker_outline_, true);
  node.param("display_marker_id", display_marker_id_, true);
  node.param("display_marker_edges", display_marker_edges_, true);
  node.param("display_marker_axes", display_marker_axes_, true);
  node.param("display_marker_axes", display_marker_axes_, true);

  node.param("TagFamily", TagFamily_, 2);
  node.param("USE_UMICH", USE_UMICH_, false);
  node.param("num_threads", nthreads_, 1);
  node.param("black_border", black_border_, 1);
  node.param("quad_decimate", quad_decimate_, 1.0);
  //强枚举类型
  ...
  //订阅相机info
  camera_info_sub = node.subscribe(camera_info_topic_name_, 1, InfoCallback);
  //订阅图像相机
  image_sub = image.subscribe(camera_topic_name_, 1, ImageCallback);
  //向RVIZ发布标记信息
  marker_pub = node.advertise<visualization_msgs::MarkerArray>("marker_array", 1);
  //发送apriltag检测信息 //用于测试
  apriltag_publisher = node.advertise<dbrobot_msg::QRTagDetections>("detections", 1);

  has_camera_info_ = false;
  ros::Rate loop_rate(process_hz_);  //每秒10帧  决定CPU的占用
  while (node.ok()) {
    ros::spinOnce();    //在订阅某个话题的时候，执行这条语句意味着只处理一次回调
    loop_rate.sleep();  //控制发送频率
  }
  ROS_INFO("april Tags node stopped.");
  return EXIT_SUCCESS;
}
